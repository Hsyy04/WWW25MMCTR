...
if [ ! -f "data/MicroLens_1M_x1/EMB_all_emb_cb05_std_data.parquet" ]; then
    python run_param_tuner.py --config config/EMB_all_emb_cb05_std.yaml --gpu 0 --script run_all_embedding
    if [ $? -ne 0 ]; then
        echo "train item embedding failed to execute."
        exit 1
    fi
    python encode_all_emb.py --config config/EMB_all_emb_cb05_std --expid EMB_cb_allemb_001_d1520c06 --gpu 0
    if [ $? -ne 0 ]; then
        echo "encode item embedding failed to execute."
        exit 1
    fi
    python generate_item_info.py --data_name EMB_all_emb_cb05_std_data
    if [ ! -f "data/MicroLens_1M_x1/EMB_all_emb_cb05_std_data.parquet" ]; then
